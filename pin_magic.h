...
#if defined(USE_SPARK_CORE_V0) || defined (USE_SPARK_CORE_V01) || defined (USE_SPARK_CORE_V02)
    #define SPARK_CORE
#endif
...
// Arduino Due
  ...
  #define RD_PORT A0				/*pin A0 */
  #define WR_PORT A1				/*pin A1 */
  ...
  #define read8inline(result) {  \
    RD_ACTIVE;   \
   delayMicroseconds(1);      \
   result =  digitalRead(D0)  |  (digitalRead(D1)<<1) |   (digitalRead(D2)<<2)  |  (digitalRead(D3)<<3) | \
  ...
  #define setReadDirInline() { \
   ...
   #define CS_IDLE    digitalWrite(CS_PORT,HIGH)
   #define RST_ACTIVE  digitalWrite(RST_PORT,LOW)
   #define RST_IDLE    digitalWrite(RST_PORT,HIGH)
